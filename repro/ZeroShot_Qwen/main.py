...
def load_dataset(dataset_name):
    ...
    train_lines = read_csv_lines(train_path)
    ...
    for line in train_lines[1:]:
        ...
        if len(parts) < 11:
            continue
        time_str, uid, lat, lon, poi_id, cat = parts[1], parts[5], parts[6], parts[7], parts[8], parts[10]
        poi_id = poi_id.strip()
        uid = uid.strip()
        ...
        if uid not in user_histories:
            user_histories[uid] = []
        user_histories[uid].append((poi_id, cat.strip(), time_str.strip(), lat.strip(), lon.strip()))

    # Read validation data — group by trajectory_id
    val_lines = read_csv_lines(val_path)
    trajectories = {}  # traj_id → [rows]
    for line in val_lines[1:]:
        parts = line.strip().split(',')
        if len(parts) < 11:
            continue
        traj_id, time_str, uid, lat, lon, poi_id, cat = (
            parts[3], parts[1], parts[5], parts[6], parts[7], parts[8], parts[10]
        )
...
# ─── Metrics ──────────────────────────────────────────────────────────────────
def compute_ndcg(ranked_list, gt_id, k):
    """Compute NDCG@k given a ranked list of candidate IDs and the ground truth ID."""
    for i, pid in enumerate(ranked_list):
        if pid == gt_id and i < k:
            return 1.0 / (i + 1)  # DCG = 1/log2(i+2), simplified: DCG@k = 1/log2(rank+1)
    return 0.0


def compute_metrics(all_results, total_samples):
    """Compute all metrics from per-sample results."""
    hit1 = hit5 = hit10 = 0
    rr_sum = 0.0
    ndcg5_sum = 0.0
    ndcg10_sum = 0.0
    ...
    for res in all_results:
        ...
        rank = res["rank"]  # 1-based rank where GT was found, None if not found
        ...
        if rank is not None:
            ...
            # NDCG: simplified — the ranked list is the order of predictions
            # In iterative guessing, our "ranked list" = predictions in order
            # which is [pred1, pred2, ..., predN] until hit or exhausted
            ndcg5_sum += compute_ndcg(res["predictions"], res["ground_truth"], 5)
            ndcg10_sum += compute_ndcg(res["predictions"], res["ground_truth"], 10)

    return {
        "total_samples": total_samples,
        "Acc@1": hit1 / total_samples,
        "Acc@5": hit5 / total_samples,
        "Acc@10": hit10 / total_samples,
        "MRR": rr_sum / total_samples,
        "NDCG@5": ndcg5_sum / total_samples,
        "NDCG@10": ndcg10_sum / total_samples,
        "ValidRatio": valid_count / total_samples,
...
def evaluate(args):
    ...
    # Exclude trajectories where user has fewer than 2 distinct categories
    # (long-term preference would be meaningless: 100% single category)
    valid_sample_ids = []
    for tid in filtered:
...
def evaluate_one(tid, traj_rows, user_histories, poi_info, all_poi_ids, client, model, output_dir, dataset):
    ...
    while remaining:
        ...
        if predicted_id not in remaining:
            # Invalid output: retry (up to limit), then give up
            invalid_retries += 1
    ...
    # If we exhausted remaining without hitting, record miss
    result = {
        "trajectory_id": tid,
        "ground_truth": gt_poi,
        "valid": True,
        "rank": len(predictions) if early_stop else None,
...
def main():
    ...
    results_csv = os.path.join(_PROJECT_ROOT, 'results', 'results.csv')
    ...
    row = (
        ...
        f'{metrics["MRR"]:.4f},{metrics["ValidRatio"]:.4f},'
        f'{timestamp},{notes}\n'
    )

    os.makedirs(os.path.dirname(results_csv), exist_ok=True)
    header = 'paper,dataset,Acc@1,Acc@5,Acc@10,MRR,ValidRatio,timestamp,notes\n'
